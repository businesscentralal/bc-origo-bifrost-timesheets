...
codeunit 10036789 "Clockify Request Mgt ori"
{
    ...
    var
        MissingWorkspaceErr: Label 'No workspace was supplied. Provide ''workspaceId'' in the request or set a Default Workspace ID on Clockify Setup.', Locked = true;
    ...
    /// Bifrost argument. A 2xx response yields <c>status = Success</c>; otherwise
    /// <c>status = Error</c> with an <c>error</c> message and the original status code.
    /// </summary>
    /// <param name="Argument">The Bifrost message argument that receives the response.</param>
    ...
    procedure Execute(var Argument: Record "Message Argument ori"; Method: Text; ResourcePath: Text; HasBody: Boolean; RequestBody: Text)
    var
    ...
    /// falling back to the Default Workspace ID on Clockify Setup. On failure
    ...
    procedure ResolveWorkspaceId(var Argument: Record "Message Argument ori"; RequestJson: JsonObject; var WorkspaceId: Text): Boolean
    begin
    ...
    /// <c>Clockify Setup</c>. When no setup record exists the enum default
    ...
    begin
        if not ClockifySetup.Get() then
            ClockifySetup.Init();
        exit(ClockifySetup."API Version");
    end;

    /// <summary>Returns the configured default Clockify workspace ID, or an empty string.</summary>
    /// <returns>The default workspace ID from Clockify Setup.</returns>
    ...
    procedure RequireBody(var Argument: Record "Message Argument ori"; RequestJson: JsonObject; var BodyText: Text): Boolean
    var
        Token: JsonToken;
    begin
