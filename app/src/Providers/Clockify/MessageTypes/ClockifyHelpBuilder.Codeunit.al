...
codeunit 10036799 "Clockify Help Builder ori"
{
    ...
    /// </summary>
    /// <param name="MessageType">The full message type name (e.g. 'Provider.Clockify.Client.Create').</param>
    ...
    begin
        ...
        // Footer
        Builder.AppendLine('---');
        Builder.AppendLine('Full integration-table reference and entity model: request help for `Provider.Clockify.Help.Get`.');
    ...
    var
        ...
        PartCount: Integer;
    begin
        // Entity + action are the last two dotted segments, so both 3-part
        // (Timesheets.TimeSheet.Create) and 4-part (Provider.Clockify.Client.Update) names parse correctly.
        Parts := MessageType.Split('.');
        PartCount := Parts.Count();
        if PartCount >= 1 then
            ActionName := Parts.Get(PartCount);
        if PartCount >= 2 then
            Entity := Parts.Get(PartCount - 1);
    end;
