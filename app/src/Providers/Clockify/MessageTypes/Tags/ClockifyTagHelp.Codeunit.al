...
codeunit 10036851 "Clockify Tag Help ori"
{
    ...
    begin
        case MessageType of
            MessageType::"Provider.Clockify.Tag.List":
                exit(GetTagListHelp(Description));
            MessageType::"Provider.Clockify.Tag.Create":
                exit(GetTagCreateHelp(Description));
            MessageType::"Provider.Clockify.Tag.Update":
                exit(GetTagUpdateHelp(Description));
            MessageType::"Provider.Clockify.Tag.Delete":
                exit(GetTagDeleteHelp(Description));
        end;
        exit('');
    end;

    /// <summary>Returns the help document for <c>Provider.Clockify.Tag.List</c>.</summary>
    /// <param name="Description">The message type description shown in the help header.</param>
    local procedure GetTagListHelp(Description: Text): Text
    var
        HelpBuilder: Codeunit "Clockify Help Builder ori";
    begin
        HelpBuilder.Init('Provider.Clockify.Tag.List', Description, 'GET', '/workspaces/{workspaceId}/tags');
        ...
        HelpBuilder.SetRelated('- **Create tag:** `Provider.Clockify.Tag.Create`\' +
            '- **Use in time entries:** Pass `id` values in `tagIds` array of `Provider.Clockify.TimeEntry.Create`/`Update`\' +
            '- **ID resolution:** Match on `name` to find the `id` needed for write operations');
        exit(HelpBuilder.Render());
    end;

    /// <summary>Returns the help document for <c>Provider.Clockify.Tag.Create</c>.</summary>
    /// <param name="Description">The message type description shown in the help header.</param>
    local procedure GetTagCreateHelp(Description: Text): Text
    var
        HelpBuilder: Codeunit "Clockify Help Builder ori";
    begin
        HelpBuilder.Init('Provider.Clockify.Tag.Create', Description, 'POST', '/workspaces/{workspaceId}/tags');
        HelpBuilder.AddParam('workspaceId', true, 'string', 'Target workspace ID', 'Provider.Clockify.Workspace.List → id');
        ...
        HelpBuilder.AddError(400, 'Tag name already exists', 'Use `Provider.Clockify.Tag.List` to find existing tag');
        HelpBuilder.AddError(401, 'Unauthorized', 'Check API key on Clockify Setup');
        ...
        HelpBuilder.SetRelated('- **List existing tags:** `Provider.Clockify.Tag.List`\' +
            '- **Use in time entries:** Pass tag `id` in `tagIds` array of `Provider.Clockify.TimeEntry.Create`/`Update`\' +
            '- **Delete later:** `Provider.Clockify.Tag.Delete` (no archive step needed)');
        exit(HelpBuilder.Render());
    end;

    /// <summary>Returns the help document for <c>Provider.Clockify.Tag.Update</c>.</summary>
    /// <param name="Description">The message type description shown in the help header.</param>
    local procedure GetTagUpdateHelp(Description: Text): Text
    var
        HelpBuilder: Codeunit "Clockify Help Builder ori";
    begin
        HelpBuilder.Init('Provider.Clockify.Tag.Update', Description, 'PUT', '/workspaces/{workspaceId}/tags/{tagId}');
        HelpBuilder.AddParam('workspaceId', true, 'string', 'Target workspace ID', 'Provider.Clockify.Workspace.List → id');
        HelpBuilder.AddParam('tagId', true, 'string', 'The tag ID to update', 'Provider.Clockify.Tag.List → id');
        ...
        HelpBuilder.AddError(404, 'Tag not found', 'Verify tagId via `Provider.Clockify.Tag.List`');
        HelpBuilder.AddError(401, 'Unauthorized', 'Check API key on Clockify Setup');
        HelpBuilder.SetNotes('- Send only fields you want to change; omitted fields retain current values.\' +
            '- Unlike clients/projects, archiving a tag is NOT required before `Provider.Clockify.Tag.Delete`.');
        HelpBuilder.SetRelated('- **Resolve tagId:** `Provider.Clockify.Tag.List` or Clockify Integration (type=tag)\' +
            '- **Delete tag:** `Provider.Clockify.Tag.Delete` (no archive step needed)');
        exit(HelpBuilder.Render());
    end;

    /// <summary>Returns the help document for <c>Provider.Clockify.Tag.Delete</c>.</summary>
    /// <param name="Description">The message type description shown in the help header.</param>
    local procedure GetTagDeleteHelp(Description: Text): Text
    var
        HelpBuilder: Codeunit "Clockify Help Builder ori";
    begin
        HelpBuilder.Init('Provider.Clockify.Tag.Delete', Description, 'DELETE', '/workspaces/{workspaceId}/tags/{tagId}');
        HelpBuilder.AddParam('workspaceId', true, 'string', 'Target workspace ID', 'Provider.Clockify.Workspace.List → id');
        HelpBuilder.AddParam('tagId', true, 'string', 'The tag ID to delete', 'Provider.Clockify.Tag.List → id');
        ...
        HelpBuilder.SetRelated('- **Alternative to delete:** Archive via `Provider.Clockify.Tag.Update` with `{ "archived": true }`\' +
