...
codeunit 10036850 "Clockify Task Help ori"
{
    ...
    begin
        case MessageType of
            MessageType::"Provider.Clockify.Task.List":
                exit(GetTaskListHelp(Description));
            MessageType::"Provider.Clockify.Task.Create":
                exit(GetTaskCreateHelp(Description));
            MessageType::"Provider.Clockify.Task.Update":
                exit(GetTaskUpdateHelp(Description));
            MessageType::"Provider.Clockify.Task.Delete":
                exit(GetTaskDeleteHelp(Description));
        end;
        exit('');
    end;

    /// <summary>Returns the help document for <c>Provider.Clockify.Task.List</c>.</summary>
    /// <param name="Description">The message type description shown in the help header.</param>
    local procedure GetTaskListHelp(Description: Text): Text
    var
        HelpBuilder: Codeunit "Clockify Help Builder ori";
    begin
        HelpBuilder.Init('Provider.Clockify.Task.List', Description, 'GET', '/workspaces/{workspaceId}/projects/{projectId}/tasks');
        HelpBuilder.AddParam('workspaceId', true, 'string', 'Target workspace ID', 'Provider.Clockify.Workspace.List → id');
        HelpBuilder.AddParam('projectId', true, 'string', 'Parent project ID', 'Provider.Clockify.Project.List → id');
        ...
        HelpBuilder.SetRelated('- **Create task:** `Provider.Clockify.Task.Create`\' +
            '- **Use in time entries:** Pass task `id` as `taskId` in `Provider.Clockify.TimeEntry.Create`\' +
            '- **Update task:** `Provider.Clockify.Task.Update`');
        exit(HelpBuilder.Render());
    end;

    /// <summary>Returns the help document for <c>Provider.Clockify.Task.Create</c>.</summary>
    /// <param name="Description">The message type description shown in the help header.</param>
    local procedure GetTaskCreateHelp(Description: Text): Text
    var
        HelpBuilder: Codeunit "Clockify Help Builder ori";
    begin
        HelpBuilder.Init('Provider.Clockify.Task.Create', Description, 'POST', '/workspaces/{workspaceId}/projects/{projectId}/tasks');
        HelpBuilder.AddParam('workspaceId', true, 'string', 'Target workspace ID', 'Provider.Clockify.Workspace.List → id');
        HelpBuilder.AddParam('projectId', true, 'string', 'Parent project ID (tasks belong to a project)', 'Provider.Clockify.Project.List → id');
        ...
        HelpBuilder.AddParam('body.assigneeIds', false, 'array', 'Array of user IDs to assign', 'Provider.Clockify.User.List → id');
        ...
        HelpBuilder.AddError(400, 'Task name already exists in project', 'Choose a different name or find existing via `Provider.Clockify.Task.List`');
        HelpBuilder.AddError(404, 'Project not found', 'Verify projectId via `Provider.Clockify.Project.List`');
        HelpBuilder.AddError(401, 'Unauthorized', 'Check API key on Clockify Setup');
        ...
        HelpBuilder.SetRelated('- **Resolve projectId:** `Provider.Clockify.Project.List` or Clockify Integration (type=project)\' +
            '- **List existing tasks:** `Provider.Clockify.Task.List`\' +
            '- **Use in time entries:** Pass task `id` as `taskId` in `Provider.Clockify.TimeEntry.Create`');
        exit(HelpBuilder.Render());
    end;

    /// <summary>Returns the help document for <c>Provider.Clockify.Task.Update</c>.</summary>
    /// <param name="Description">The message type description shown in the help header.</param>
    local procedure GetTaskUpdateHelp(Description: Text): Text
    var
        HelpBuilder: Codeunit "Clockify Help Builder ori";
    begin
        HelpBuilder.Init('Provider.Clockify.Task.Update', Description, 'PUT', '/workspaces/{workspaceId}/projects/{projectId}/tasks/{taskId}');
        HelpBuilder.AddParam('workspaceId', true, 'string', 'Target workspace ID', 'Provider.Clockify.Workspace.List → id');
        HelpBuilder.AddParam('projectId', true, 'string', 'Parent project ID', 'Provider.Clockify.Project.List → id');
        HelpBuilder.AddParam('taskId', true, 'string', 'The task ID to update', 'Provider.Clockify.Task.List → id');
        ...
        HelpBuilder.AddParam('body.assigneeIds', false, 'array', 'Array of user IDs', 'Provider.Clockify.User.List → id');
        HelpBuilder.AddParam('body.billable', false, 'boolean', 'Override project billable default', '');
        ...
        HelpBuilder.AddError(404, 'Task or project not found', 'Verify IDs via `Provider.Clockify.Task.List`');
        HelpBuilder.AddError(401, 'Unauthorized', 'Check API key on Clockify Setup');
        ...
        HelpBuilder.SetRelated('- **Resolve taskId:** `Provider.Clockify.Task.List` or Clockify Integration (type=task)\' +
            '- **Delete task:** `Provider.Clockify.Task.Delete` (no archive step needed for tasks)');
        exit(HelpBuilder.Render());
    end;

    /// <summary>Returns the help document for <c>Provider.Clockify.Task.Delete</c>.</summary>
    /// <param name="Description">The message type description shown in the help header.</param>
    local procedure GetTaskDeleteHelp(Description: Text): Text
    var
        HelpBuilder: Codeunit "Clockify Help Builder ori";
    begin
        HelpBuilder.Init('Provider.Clockify.Task.Delete', Description, 'DELETE', '/workspaces/{workspaceId}/projects/{projectId}/tasks/{taskId}');
        HelpBuilder.AddParam('workspaceId', true, 'string', 'Target workspace ID', 'Provider.Clockify.Workspace.List → id');
        HelpBuilder.AddParam('projectId', true, 'string', 'Parent project ID', 'Provider.Clockify.Project.List → id');
        HelpBuilder.AddParam('taskId', true, 'string', 'The task ID to delete', 'Provider.Clockify.Task.List → id');
        ...
        HelpBuilder.SetRelated('- **Alternative to delete:** Set status to DONE via `Provider.Clockify.Task.Update`\' +
