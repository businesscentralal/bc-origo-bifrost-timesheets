...
/// <summary>
/// Builds the Markdown help documents for the <c>Timesheets.TimeSheet.*</c> message types,
...
codeunit 10036838 "Clockify TimeSheet Help ori"
{
    ...
    begin
        case MessageType of
            MessageType::"Timesheets.TimeSheet.Create":
                exit(GetCreateHelp(Description));
            MessageType::"Timesheets.TimeSheet.Approve":
                exit(GetApproveHelp(Description));
            MessageType::"Timesheets.TimeSheet.Post":
                exit(GetPostHelp(Description));
            MessageType::"Timesheets.TimeSheet.Archive":
                exit(GetArchiveHelp(Description));
            MessageType::"Timesheets.TimeSheet.Reject":
                exit(GetRejectHelp(Description));
            MessageType::"Timesheets.TimeSheet.Reopen":
                exit(GetReopenHelp(Description));
            MessageType::"Timesheets.TimeSheet.SyncFromClockify":
                exit(GetSyncToTimeSheetHelp(Description));
            MessageType::"Timesheets.TimeSheet.SyncRangeFromClockify":
                exit(GetSyncRangeToTimeSheetHelp(Description));
            MessageType::"Timesheets.SyncAllUsersFromClockify":
                exit(GetSyncAllUsersHelp(Description));
        end;
        exit('');
    end;

    /// <summary>Returns the help document for <c>Timesheets.TimeSheet.Create</c>.</summary>
    local procedure GetCreateHelp(Description: Text): Text
    var
        HelpBuilder: Codeunit "Clockify Help Builder ori";
    begin
        HelpBuilder.Init('Timesheets.TimeSheet.Create', Description, 'POST (BC-side)', '/internal/timesheet-create');
    ...
    /// <summary>Returns the help document for <c>Timesheets.TimeSheet.Approve</c>.</summary>
    local procedure GetApproveHelp(Description: Text): Text
    var
        HelpBuilder: Codeunit "Clockify Help Builder ori";
    begin
        HelpBuilder.Init('Timesheets.TimeSheet.Approve', Description, 'POST (BC-side)', '/internal/timesheet-approve');
    ...
    /// <summary>Returns the help document for <c>Timesheets.TimeSheet.Post</c>.</summary>
    local procedure GetPostHelp(Description: Text): Text
    var
        HelpBuilder: Codeunit "Clockify Help Builder ori";
    begin
        HelpBuilder.Init('Timesheets.TimeSheet.Post', Description, 'POST (BC-side)', '/internal/timesheet-post');
    ...
    /// <summary>Returns the help document for <c>Timesheets.TimeSheet.Archive</c>.</summary>
    local procedure GetArchiveHelp(Description: Text): Text
    var
        HelpBuilder: Codeunit "Clockify Help Builder ori";
    begin
        HelpBuilder.Init('Timesheets.TimeSheet.Archive', Description, 'POST (BC-side)', '/internal/timesheet-archive');
    ...
    /// <summary>Returns the help document for <c>Timesheets.TimeSheet.Reject</c>.</summary>
    local procedure GetRejectHelp(Description: Text): Text
    var
        HelpBuilder: Codeunit "Clockify Help Builder ori";
    begin
        HelpBuilder.Init('Timesheets.TimeSheet.Reject', Description, 'POST (BC-side)', '/internal/timesheet-reject');
    ...
    /// <summary>Returns the help document for <c>Timesheets.TimeSheet.Reopen</c>.</summary>
    local procedure GetReopenHelp(Description: Text): Text
    var
        HelpBuilder: Codeunit "Clockify Help Builder ori";
    begin
        HelpBuilder.Init('Timesheets.TimeSheet.Reopen', Description, 'POST (BC-side)', '/internal/timesheet-reopen');
    ...
    /// <summary>Returns the help document for <c>Timesheets.TimeSheet.SyncFromClockify</c>.</summary>
    local procedure GetSyncToTimeSheetHelp(Description: Text): Text
    var
        HelpBuilder: Codeunit "Clockify Help Builder ori";
    begin
        HelpBuilder.Init('Timesheets.TimeSheet.SyncFromClockify', Description, 'POST (BC-side)', '/internal/sync-time-entry-timesheet');
        HelpBuilder.AddParam('workspaceId', true, 'string', 'Source workspace ID', 'Provider.Clockify.Workspace.List → id');
        HelpBuilder.AddParam('userId', true, 'string', 'Clockify user ID (mapped to a BC Resource)', 'Provider.Clockify.User.GetCurrent → id');
        HelpBuilder.AddParam('entryId', true, 'string', 'Clockify time entry ID', 'Provider.Clockify.TimeEntry.List → id');
        HelpBuilder.AddParam('projectId', true, 'string', 'Clockify project ID (mapped to BC Job No.)', 'Provider.Clockify.Project.List → id');
        HelpBuilder.AddParam('taskId', true, 'string', 'Clockify task ID (mapped to BC Job Task No.)', 'Provider.Clockify.Task.List → id');
        ...
        HelpBuilder.AddParam('tagIds', false, 'array', 'Clockify tag IDs used to resolve the Work Type', 'Provider.Clockify.Tag.List → id');
        ...
        HelpBuilder.SetPreconditions('1. The Clockify project/task/user must be mapped in the Clockify Integration table.\' +
            '2. The resource must have an **open time sheet covering the entry date** — run `Timesheets.TimeSheet.Create` first.');
        HelpBuilder.AddError(400, 'Missing mapping', 'Ensure project/task/user mappings exist in the Clockify Integration table');
        HelpBuilder.AddError(400, 'No open time sheet', 'Run `Timesheets.TimeSheet.Create`, then retry');
        HelpBuilder.SetNotes('- Writes to a **BC Time Sheet** (line + detail), not the Job Journal.\' +
            ...
            '- After approval, post the sheet with `Timesheets.TimeSheet.Post`.');
        HelpBuilder.SetRelated('- **Populate sheets:** `Timesheets.TimeSheet.Create`\' +
            '- **Approve/post:** `Timesheets.TimeSheet.Approve` → `Timesheets.TimeSheet.Post`\' +
            '- **Journal-direct alternative:** `Timesheets.JobJournal.SyncFromClockify`');
        exit(HelpBuilder.Render());
    end;

    /// <summary>Returns the help document for <c>Timesheets.TimeSheet.SyncRangeFromClockify</c>.</summary>
    local procedure GetSyncRangeToTimeSheetHelp(Description: Text): Text
    var
        HelpBuilder: Codeunit "Clockify Help Builder ori";
    begin
        HelpBuilder.Init('Timesheets.TimeSheet.SyncRangeFromClockify', Description, 'POST (BC-side)', '/internal/sync-time-entries-timesheet');
        HelpBuilder.AddParam('workspaceId', true, 'string', 'Source workspace ID', 'Provider.Clockify.Workspace.List → id');
        HelpBuilder.AddParam('userId', true, 'string', 'Clockify user ID whose entries to sync', 'Provider.Clockify.User.GetCurrent → id');
        ...
        HelpBuilder.SetPreconditions('1. Project/task/user mappings must exist in the Clockify Integration table.\' +
            '2. Each entry date must fall within an **open time sheet** for the resource — run `Timesheets.TimeSheet.Create` first.\' +
            '3. Only finished entries are synced (running timers are excluded).');
        HelpBuilder.SetNotes('- Reads entries from Clockify (paged), then writes **BC Time Sheets** — combines `Provider.Clockify.TimeEntry.List` + `Timesheets.TimeSheet.SyncFromClockify`.\' +
        ...
        HelpBuilder.SetRelated('- **Populate sheets:** `Timesheets.TimeSheet.Create`\' +
            '- **Approve/post:** `Timesheets.TimeSheet.Approve` → `Timesheets.TimeSheet.Post`\' +
            '- **Journal-direct alternative:** `Timesheets.JobJournal.SyncRangeFromClockify`');
        exit(HelpBuilder.Render());
    end;

    /// <summary>Returns the help document for <c>Timesheets.SyncAllUsersFromClockify</c>.</summary>
    local procedure GetSyncAllUsersHelp(Description: Text): Text
    var
        HelpBuilder: Codeunit "Clockify Help Builder ori";
    begin
        HelpBuilder.Init('Timesheets.SyncAllUsersFromClockify', Description, 'POST (BC-side)', '/internal/sync-all-users');
        HelpBuilder.AddParam('workspaceId', true, 'string', 'Source workspace ID', 'Provider.Clockify.Workspace.List → id');
        HelpBuilder.AddParam('start', true, 'string', 'Range start in ISO-8601 UTC (inclusive)', '');
        HelpBuilder.AddParam('end', true, 'string', 'Range end in ISO-8601 UTC (inclusive)', '');
        ...
        HelpBuilder.SetPreconditions('1. Each Clockify user must be mapped to a BC Resource (USER integration row).\' +
            '2. For target=timesheet, each entry date needs an open time sheet — run `Timesheets.TimeSheet.Create` first.\' +
        ...
        HelpBuilder.SetNotes('- Restores the legacy per-user auto-discovery: iterates every USER mapping, no need to pass `userId`.\' +
            ...
            '- Safe to re-run: unchanged entries return `Skipped`.');
        HelpBuilder.SetRelated('- **Single user, time sheet:** `Timesheets.TimeSheet.SyncRangeFromClockify`\' +
            '- **Single user, journal:** `Timesheets.JobJournal.SyncRangeFromClockify`\' +
            '- **Seed sheets:** `Timesheets.TimeSheet.Create`');
        exit(HelpBuilder.Render());
    end;
}
