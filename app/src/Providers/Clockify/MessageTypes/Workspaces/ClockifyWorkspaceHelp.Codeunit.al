...
/// </summary>
codeunit 10036847 "Clockify Workspace Help ori"
{
    Access = Internal;
    ...
    begin
        case MessageType of
            MessageType::"Provider.Clockify.Workspace.List":
                exit(GetWorkspaceListHelp(Description));
            MessageType::"Provider.Clockify.User.GetCurrent":
                exit(GetUserGetCurrentHelp(Description));
            MessageType::"Provider.Clockify.User.List":
                exit(GetUserListHelp(Description));
            MessageType::"Provider.Clockify.Currency.List":
                exit(GetCurrencyListHelp(Description));
            MessageType::"Provider.Clockify.UserGroup.List":
                exit(GetUserGroupListHelp(Description));
            MessageType::"Provider.Clockify.CustomField.List":
                exit(GetCustomFieldListHelp(Description));
        end;
        exit('');
    end;

    /// <summary>Returns the help document for <c>Provider.Clockify.Workspace.List</c>.</summary>
    /// <param name="Description">The message type description shown in the help header.</param>
    local procedure GetWorkspaceListHelp(Description: Text): Text
    var
        HelpBuilder: Codeunit "Clockify Help Builder ori";
    begin
        HelpBuilder.Init('Provider.Clockify.Workspace.List', Description, 'GET', '/workspaces');
        ...
        HelpBuilder.SetRelated('- **Get current user:** `Provider.Clockify.User.GetCurrent` (also returns `activeWorkspace`)\' +
            '- **All other operations:** Pass workspace `id` as `workspaceId` parameter');
        exit(HelpBuilder.Render());
    end;

    /// <summary>Returns the help document for <c>Provider.Clockify.User.GetCurrent</c>.</summary>
    /// <param name="Description">The message type description shown in the help header.</param>
    local procedure GetUserGetCurrentHelp(Description: Text): Text
    var
        HelpBuilder: Codeunit "Clockify Help Builder ori";
    begin
        HelpBuilder.Init('Provider.Clockify.User.GetCurrent', Description, 'GET', '/user');
        HelpBuilder.SetRequestExample('{ }');
        ...
        HelpBuilder.SetNotes('- No parameters needed — returns the user who owns the API key.\' +
            '- The `id` field is the `userId` required by `Provider.Clockify.TimeEntry.Create`, `Provider.Clockify.TimeEntry.List`, etc.\' +
            '- `activeWorkspace` is the user''s currently selected workspace ID.');
        HelpBuilder.SetRelated('- **List all users in workspace:** `Provider.Clockify.User.List`\' +
            '- **Use userId for time entries:** `Provider.Clockify.TimeEntry.Create`, `Provider.Clockify.TimeEntry.List`');
        exit(HelpBuilder.Render());
    end;

    /// <summary>Returns the help document for <c>Provider.Clockify.User.List</c>.</summary>
    /// <param name="Description">The message type description shown in the help header.</param>
    local procedure GetUserListHelp(Description: Text): Text
    var
        HelpBuilder: Codeunit "Clockify Help Builder ori";
    begin
        HelpBuilder.Init('Provider.Clockify.User.List', Description, 'GET', '/workspaces/{workspaceId}/users');
        ...
        HelpBuilder.SetNotes('- Returns all members of the workspace, including pending invitations.\' +
            '- The `id` field is the `userId` needed for `Provider.Clockify.TimeEntry.Create/List` and `memberships` in project/task operations.');
        HelpBuilder.SetRelated('- **Get API key owner only:** `Provider.Clockify.User.GetCurrent`\' +
            '- **Assign to projects:** Use `id` in `memberships` array of `Provider.Clockify.Project.Create/Update`\' +
            '- **Map to BC Resource:** Store mapping in Clockify Integration table (type=user)');
        exit(HelpBuilder.Render());
    end;

    /// <summary>Returns the help document for <c>Provider.Clockify.Currency.List</c>.</summary>
    /// <param name="Description">The message type description shown in the help header.</param>
    local procedure GetCurrencyListHelp(Description: Text): Text
    var
        HelpBuilder: Codeunit "Clockify Help Builder ori";
    begin
        HelpBuilder.Init('Provider.Clockify.Currency.List', Description, 'GET', '/workspaces (inline `currencies` from workspace object)');
        HelpBuilder.AddParam('workspaceId', true, 'string', 'Target workspace ID', 'Provider.Clockify.Workspace.List → id');
        ...
        HelpBuilder.SetNotes('- Clockify has no standalone currencies endpoint. This message type reads `GET /workspaces` and extracts the requested workspace''s `currencies` array.\' +
            ...
            '- Use the `id` value (NOT the `code`) as `currencyId` in `Provider.Clockify.Client.Create` / `Provider.Clockify.Client.Update`. Passing `currencyCode` is silently ignored.\' +
            '- The list is per-workspace — the same currency code can have different IDs in different workspaces.');
        HelpBuilder.SetRelated('- **Use currencyId:** `Provider.Clockify.Client.Create` and `Provider.Clockify.Client.Update` (body.currencyId)\' +
            '- **Workspace info:** `Provider.Clockify.Workspace.List` returns the full workspace object including currencies');
        exit(HelpBuilder.Render());
    end;

    /// <summary>Returns the help document for <c>Provider.Clockify.UserGroup.List</c>.</summary>
    /// <param name="Description">The message type description shown in the help header.</param>
    local procedure GetUserGroupListHelp(Description: Text): Text
    var
        HelpBuilder: Codeunit "Clockify Help Builder ori";
    begin
        HelpBuilder.Init('Provider.Clockify.UserGroup.List', Description, 'GET', '/workspaces/{workspaceId}/user-groups');
        HelpBuilder.AddParam('workspaceId', true, 'string', 'Target workspace ID', 'Provider.Clockify.Workspace.List → id');
        HelpBuilder.AddParam('query.page-size', false, 'integer', 'Results per page (default 50, max 5000)', '');
        HelpBuilder.AddParam('query.page', false, 'integer', 'Page number (1-based)', '');
        HelpBuilder.AddParam('query.name', false, 'string', 'Filter: partial name match', '');
        HelpBuilder.AddParam('query.projectId', false, 'string', 'Filter: groups assigned to this project', 'Provider.Clockify.Project.List → id');
        HelpBuilder.SetRequestExample('{ "workspaceId": "5f...", "query": { "page-size": 50, "page": 1 } }');
        ...
        HelpBuilder.SetNotes('- Each group has a `userIds` array listing member Clockify user IDs.\' +
            '- Use the `id` value as a member of `userGroupIds` in `Provider.Clockify.Project.Create` / `Provider.Clockify.Project.Update`. Group **names** are not accepted on those write paths.');
        HelpBuilder.SetRelated('- **Assign group to project:** `Provider.Clockify.Project.Create` or `Provider.Clockify.Project.Update` (body.userGroupIds)\' +
            '- **List group members:** Check `userIds` array in response; resolve to names via `Provider.Clockify.User.List`');
        exit(HelpBuilder.Render());
    end;

    /// <summary>Returns the help document for <c>Provider.Clockify.CustomField.List</c>.</summary>
    /// <param name="Description">The message type description shown in the help header.</param>
    local procedure GetCustomFieldListHelp(Description: Text): Text
    var
        HelpBuilder: Codeunit "Clockify Help Builder ori";
    begin
        HelpBuilder.Init('Provider.Clockify.CustomField.List', Description, 'GET', '/workspaces/{workspaceId}/custom-fields');
        HelpBuilder.AddParam('workspaceId', true, 'string', 'Target workspace ID', 'Provider.Clockify.Workspace.List → id');
        HelpBuilder.AddParam('query.page-size', false, 'integer', 'Results per page (default 50, max 5000)', '');
        HelpBuilder.AddParam('query.page', false, 'integer', 'Page number (1-based)', '');
        ...
        HelpBuilder.SetNotes('- Field `type` values: `TXT`, `NUMBER`, `DROPDOWN_SINGLE`, `DROPDOWN_MULTIPLE`, `CHECKBOX`, `LINK`.\' +
            '- Use the `id` value as `customFieldId` in `Provider.Clockify.TimeEntry.Create/Update` and `Provider.Clockify.Project.Update` bodies.\' +
            '- For per-project enablement and defaults, send `customFieldId` inside the `customFields` array on `Provider.Clockify.Project.Update`.\' +
            '- This endpoint returns **workspace-level** definitions only. Project-scoped overrides (enabled/disabled, project-level defaults) surface inside `Provider.Clockify.Project.Get`.');
        HelpBuilder.SetRelated('- **Use on time entries:** `Provider.Clockify.TimeEntry.Create/Update` (body.customFields)\' +
            '- **Enable per project:** `Provider.Clockify.Project.Update` (body.customFields)\' +
            '- **See project overrides:** `Provider.Clockify.Project.Get` (response.customFields)');
        exit(HelpBuilder.Render());
    end;
}
